...
def get_economic_indicators():
    # Fetch interest rates from Federal Reserve
    interest_rate_url = 'https://api.stlouisfed.org/fred/series/FDTR/observations?api_key=YOUR_API_KEY&file_type=json'
    try:
        response = requests.get(interest_rate_url)
        data = response.json()
        current_interest_rate = float(data['observations'][-1]['value'])  # Get the most recent interest rate
    ...
    # For inflation, using CPI data as a proxy
    cpi_url = 'https://api.stlouisfed.org/fred/series/CPIAUCNS/observations?api_key=YOUR_API_KEY&file_type=json'
    try:
        response = requests.get(cpi_url)
        data = response.json()
        current_cpi = float(data['observations'][-1]['value'])  # Get the most recent CPI
